...
func TestLrc2subRip(t *testing.T) {
	tests := []struct {
		lrcLyrics  []string
		songLength string
		x          []string
		...
		{
			[]string{"[00:12.00] Happy birthday dear coder,", "[00:17.20] Happy birthday to you!"},
			"00:00:20",
			[]string{"1",
		...
		{
			[]string{"[00:04.42] jingle bells, jingle bells, jingle all the way!",
		...
		{
			[]string{"[95:19.55] i hear babies cryin,",
		...
		{
			[]string{"[12:24.80] make it the cutest",
		...
		{
			[]string{"[12:51.10] Hello tacher tell me whats my lesson",
	...
	for i, test := range tests {
		actual := lrc2subRip(test.lrcLyrics, test.songLength)

		if !reflect.DeepEqual(actual, test.x) {
			t.Errorf("Test %d bonked", i+1)
			// fmt.Println("actual", actual)
			for i, line := range actual {
				// fmt.Println(line)
				if line != test.x[i] {
					fmt.Printf("Line %d is wrong.\n Got %s\n Expected %s\n", i+1, line, test.x[i])
				}
			}
		}
	}
